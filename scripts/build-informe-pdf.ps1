# Genera Informe-Practica-OSINT.pdf desde el Markdown (capturas locales embebidas).
...
& $pandoc $MdTmp -o $HtmlOut --standalone --embed-resources --resource-path="." -V lang=es --metadata title="Reconocimiento DNS - Grupo GEE"
...
Start-Process -FilePath $edge -ArgumentList @(
    ...
    "--print-to-pdf=`"$pdfFull`"", $uri
) -Wait -NoNewWindow | Out-Null

Start-Sleep -Seconds 2
if (-not (Test-Path $pdfFull)) { throw "No se generó $PdfOut" }
